...
const MeetingForm = ({ meeting, onSave, onCancel, isEditMode }) => {
  ...
  const [dialogContent, setDialogContent] = useState(''); // Content for confirmation dialog
...
useEffect(() => {
    if (meeting) {
        setFormData({
            ...
            teacherId: Number(meeting.teacherId),
            roomId: Number(meeting.roomId),
            startTime: meeting.startTime,
            endTime: meeting.endTime,
            meetingDate: meeting.meetingDate ? dayjs(meeting.meetingDate).format("YYYY-MM-DD") : null,
            meetingNumberForTopic: Number(meeting.meetingNumberForTopic),
            isValid: meeting.isValid !== undefined ? meeting.isValid : true,
            isPartOfSchedule: meeting.isPartOfSchedule || false,
            reason: meeting.reason || '',
            statusCourseId: Number(meeting.statusCourseId) || 0    
        });
    } else {
...
const validateForm = () => {
        ...
        if (isEditMode) {
            ...
            if (!formData.isValid) errors.isValid = 'שיבוץ תקין הוא שדה חובה';
              // סיבה: חובה רק אם השיבוץ אינו תקין
            if (!formData.isValid && !formData.reason) errors.reason = 'סיבה היא שדה חובה כששיבוץ אינו תקין';
 ...
 const isRoomAvailable = (roomId, meetingDate, startTime, endTime, existingMeetings, currentEditMode, currentMeetingId) => {
  ...
  const calculateMeetingNumber = (topicId, meetingDate, allMeetings = []) => {
    ...
    for (let i = 0; i < topicMeetings.length; i++) {
        if (new Date(topicMeetings[i].meetingDate).toDateString() === new Date(meetingDate).toDateString()) {
            count++;
            // If there are multiple meetings on the same day for the same topic,
            // this simple logic might not be sufficient if you need a unique number.
            // For now, it just counts how many there are up to this date.
        }
    }
  ...
  const isShivutzInvalid = (meeting, topic, course, room) => {
    ...
    if (topic) {
        // Assuming topic.days are 0-6 (Sun-Sat) and topic.hours are "HH:mm" strings
...
const handleSaveMeeting = async () => {
    ...
    if (Object.keys(errors).length > 0) {
        showSnackbar('אנא תקן את השגיאות בטופס');
        return;
    }
    ...
    try {
        ...
        const validationResult = { isInvalid: false, reasons: [] };
        //const meetingNumberForTopic = 1;

     if (isEditMode) {
        validationResult = isShivutzInvalid(formData, selectedTopic, selectedCourse, selectedRoom);
        ...
        if (!isRoomAvailable(
            ...
            showSnackbar('החדר אינו פנוי בתאריך ובשעות המבוקשות');
            return;
        }
      };
       ...
       if (isEditMode && validationResult.isInvalid) {
        ...
        const meetingDTO = {
          meetingId: isEditMode ? Number(formData.meetingId) : 0,
          ...
          startTime: formData.startTime + ":00", // הוספת שניות כמו בדף המקורי
          endTime: formData.endTime + ":00",     // הוספת שניות כמו בדף המקורי
          ...
          statusCourseId: formData.statusCourseId && formData.statusCourseId !== '' ? parseInt(formData.statusCourseId) : 0
        };
        ...
        showSnackbar(isEditMode ? 'המפגש עודכן בהצלחה!' : 'המפגש נוסף בהצלחה למערכת!');
